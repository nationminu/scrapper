		...
		for (UrlVO d : domains) {
			...
			try {
				...
				if (site_url.startsWith("https://www.autodaily.co.kr")) {
					...
					published_time = doc.selectFirst("meta[property=article:published_time]").attr("content")
							.toString();
				...
				if (site_url.startsWith("https://www.intelligenttransport.com")) {
					...
					published_time = String.format("%04d-%02d-%02dT00:00:00+09:00", Integer.parseInt(p_date[3]),
							Integer.parseInt(convertchartonumonths(p_date[2])), Integer.parseInt(p_date[1]));
					// p_date[3]+ "-" + convertchartonumonths(p_date[2]) + "-" +
					// String.format("%02d",Integer.parseInt(p_date[1]))+"T00:00:00+09:00";2020-12-25T20:46:07.464Z
				...
				if (site_url.startsWith("http://jmagazine.joins.com")) {
					...
					published_time = doc.selectFirst("meta[property=article:published_time]").attr("content")
							.toString();
					description = doc.select("div.con_area > span").isEmpty() == true ? ""
							: doc.select("div.con_area > span").text();
				...
				//log.info(dateformat.format(today));
				cv.setScrapeddate(dateformat.format(today));
